...
//função que capta todos os valores de todos os inputs de pesquisa, que recebe como paramentro o id do input. Feita para ser chamada em outras funções

function search(inputID) {
    const input = document.querySelector(inputID);

    if (input.value === "") {
        modal('Erro - 001', 'Caixa de texto vazia, por favor digite algo.');
    } else {
        modal('Erro - 002', 'Função temporariamente indisponivél, por favor tente novamente mais tarde.')
    }
};
...
// ouvinte input Header

(function searchHeader() {
    const button = document.querySelector(`#searchBarButton`);
...
// ouvinte input Header Mobile

(function searchHeaderMobile() {
    const button = document.querySelector(`#buttonInputHeaderMobile`);

    button.addEventListener('click', () => {
        search('#searchBarInputMobile');
    })
})();

// ouvinte input Footer

(function searchFooter() {
    const button = document.querySelector(`#buttonInputFooter`);

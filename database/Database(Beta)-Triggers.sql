...
DROP TRIGGER IF EXISTS trg_itens_venda_atualiza_venda_total;
DROP TRIGGER IF EXISTS trg_itens_venda_update_valor_total;
DROP TRIGGER IF EXISTS trg_retornar_quantidade_estoque;
DROP TRIGGER IF EXISTS trg_itens_venda_update_venda_total;
DROP TRIGGER IF EXISTS trg_itens_venda_delete_venda_total;
DROP TRIGGER IF EXISTS trg_historico_pessoas_deletadas;
DROP TRIGGER IF EXISTS historico_preco_new_old;
DROP TRIGGER IF EXISTS trg_historicos_produtos_deletados;
DROP TRIGGER IF EXISTS trg_venda_confirmada_atualiza_estoque;
DROP TRIGGER IF EXISTS trg_historico_vendas_deletadas; 
...
BEGIN
    INSERT INTO pessoas_deletadas (
        nome, email, cpf, genero, data_nascimento, data_cadastro, situacao
    )
    VALUES (
        OLD.nome, OLD.email, OLD.cpf, OLD.genero, OLD.data_nascimento, OLD.data_cadastro, OLD.situacao
    );
END;
//
DELIMITER ;
...
BEGIN
    INSERT INTO vendas_deletadas (
        desconto, acrescimo, cupom, data_compra, valor_total, situacao
    )
    VALUES (
        OLD.desconto, OLD.acrescimo, OLD.cupom, OLD.data_compra, OLD.valor_total, OLD.situacao
    );
END;
//
DELIMITER ;

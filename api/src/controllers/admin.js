...
class Adm {
    ...
    async post(req, res) {
        ...
        try {
            ...
            await connection.execute(sql, [adm.nome, adm.email, adm.cpf, adm.genero, adm.dataNascimento]);
    ...
    async put(req, res) {
        ...
        try {
            ...
            const sql_1 = "UPDATE pessoa SET nome = ?, email = ?, cpf = ?, situacao = ?, genero = ?, dataNascimento = ? WHERE id_pessoa = (SELECT id_pessoa FROM administrador WHERE id_administrador = ?)";

            await connection.execute(sql_1, [adm.nome, adm.email, adm.cpf, adm.situacao, adm.genero, adm.dataNascimento, adm.id_administrador]);
